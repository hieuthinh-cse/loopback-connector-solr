...
SolrConnector.prototype.all = function (model, filter, callback) {
    ...
    if (filter.where) {
        ...
        _.keys(where).forEach(function (p, index) {
            if (typeof where[p] === 'string') {
                solrQuery.matchFilter(p, where[p]);
                return;
            }
            _.keys(where[p]).forEach((op) => {
                if (op === 'inq') {
                    solrQuery.matchFilter(p, `(${where[p][op].join(' ')})`);
                } else if (op === 'nin') {
                    solrQuery.matchFilter(`!${p}`, `(${where[p][op].join(' ')})`);
                }
            });
...
//    };
    if (filter.facet) {
//        console.log('filter.facet: ', typeof filter.facet, filter.facet);
        solrQuery.set(querystring.stringify({
            'json.facet': JSON.stringify(filter.facet)
        }));
    }
