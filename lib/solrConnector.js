...
SolrConnector.prototype.all = function (model, filter, callback) {
	...
	_.keys(filter).forEach(function (k) {
		...
		if (typeof solrQuery[k] === 'function') {
			if (_.isArray(filter[k])) {
				filter[k].forEach(function (p) {
					debug('applying %s() with %s', k, filter[k][p]);
					solrQuery[k](filter[k][p]);
				});
			} else {
				debug('applying %s() with %s', k, filter[k]);
				solrQuery[k](filter[k]);
			}
		}
	});

	if (filter.fq) {
		_.keys(filter.fq).forEach(function (k) {
			if (_.isArray(filter.fq[k])) {
				filter.fq[k].forEach(function (p) {
					debug('applying matchFilter() with %s, %s', k, p);
					solrQuery.matchFilter(k, p);
				});
			} else {
				debug('applying matchFilter() with %s, %s', k, filter.fq[k]);
				solrQuery.matchFilter(k, filter.fq[k]);
			}
		});
	}
	...
	self.db.search(filter, function(err, data) {

		callback(err, data.response.docs);
	});
};
